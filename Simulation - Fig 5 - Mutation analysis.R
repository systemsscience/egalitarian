...
totalruns <- length(mutationprobabilities) * repetitions
...
mutationprobabilityval <- matrix(nrow=totalruns,ncol=1)

#------------------------------------------------------------------------------------
# Complete one or more runs for each parameter combination --------------------------
#------------------------------------------------------------------------------------

# this is only useful if you want to run segments of code within for loops below.
...
for (mutationprobability in mutationprobabilities) {
	...
	for(repetition in 1:repetitions) {	
		...
		cat(paste("\n run=",runcounter,"/",totalruns," v=",v," d=",d," w=",w," c=",c," b=",b,"mutationprobability=",mutationprobability," repetition=",repetition,sep=""))
		...
		cval[runcounter] <- c
		wval[runcounter] <- w
		mutationprobabilityval[runcounter] <- mutationprobability
...
} # end of for loop for this value of mutationprobability
...
temporarytable <- data.frame(rval,dval,cval,wval,bval,mutationprobabilityval,FractionDove, FractionCooperator,FractionLeveller,FractionConditionalCooperator,FractionConditionalDove, FractionComplyToLeveller,FractionComplyToConditionalCooperator,FractionComplyToConditionalDove)
names(temporarytable) <- c("v","d","c","w","b","mutationprobability","FractionDove","FractionCooperator","FractionLeveller","FractionConditionalCooperator","FractionConditionalDove","FractionComplyToLeveller","FractionComplyToConditionalCooperator","FractionComplyToConditionalDove")
results <- ddply(temporarytable,c("v","d","c","w","b","mutationprobability"),summarise,FractionDove=mean(FractionDove),FractionCooperator=mean(FractionCooperator),FractionLeveller=mean(FractionLeveller),FractionConditionalCooperator=mean(FractionConditionalCooperator),FractionConditionalDove=mean(FractionConditionalDove),FractionComplyToLeveller=mean(FractionComplyToLeveller),FractionComplyToConditionalCooperator=mean(FractionComplyToConditionalCooperator),FractionComplyToConditionalDove=mean(FractionComplyToConditionalDove))
...
points(result$mutationprobability, result$FractionCooperator,type="l",col="darkorange",lwd=3)
points(result$mutationprobability, result$FractionConditionalCooperator, type="l",col="darkorange",lty=1)
points(result$mutationprobability, result$FractionComplyToConditionalCooperator, type="l",lty=3,lwd=1,col="darkorange")
points(result$mutationprobability, result$FractionLeveller, type="l",col="green4",lwd=4,lty=1)
points(result$mutationprobability, result$FractionComplyToLeveller, type="l",lty=3,lwd=1,col="green4")
axis(2,at=c(0,1))
axis(1,at=c(0.00000001,0.0000001,0.000001,0.00001,0.0001,0.001))
